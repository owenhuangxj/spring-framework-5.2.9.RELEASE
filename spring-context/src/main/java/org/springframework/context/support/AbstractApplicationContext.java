...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			 ...
			 * todo:没看懂......
			 * 实际调用的是org.springframework.context.support.GenericApplicationContext.refreshBeanFactory
			 * 和 org.springframework.context.support.GenericApplicationContext.getBeanFactory 啥都没干，就是返
			 * GenericApplicationContext 持有的beanFactory（DefaultListableBeanFactory类型）
			 * */
			...
			try {
				 ...
				 * 实例化并按照顺序（如果显式的设置了顺序）调用所有注册为bean的bean工厂后置处理器。
				 * 必须在单例实例化之前调用。
				 */
	...
	protected void prepareRefresh() {
		 ...
		 * 这个操作是允许在多播程序可用时发布早期ApplicationEvents的集合
		 */
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		/**
		 * AbstractApplicationContext继承了DefaultResourceLoader，DefaultResourceLoader实现了ResourceLoader;
		 * ConfigurableEnvironment实现了PropertyResolver扩展点，添加一个属性编辑器的注册器,也可以使用 CustomEditorConfigurer进行设置
		 ...
		 * ApplicationContextAwareProcessor为的BeanPostProcessor实现，它为实现EnvironmentAware、EmbeddedValueResolverAware、
		 *  ResourceLoaderAware、ApplicationEventPublisherAware、MessageSourceAware和/或ApplicationContextAware接口的bean提供应
		 *  用上下文的ApplicationContext（直接持有）、Environment（通过持有ApplicationContext获取）或StringValueResolver（直接持有）。
		 *  注意：不能访问前面提到的三个属性，因为没有为这三个属性提供getter方法
		 *  */
		beanFactory.addBeanPostProcessor(new ApplicationContextAwareProcessor(this));
		/**
		 * ignoreDependencyInterface是防止形参(interface)中的属性在进行属性注入时通过setter的方式进行注入的作用，而不是忽略ifc类型的属性的注入的作用
		 * */
		 ...
		 * 当有其它对象注入BeanFactory类型的属性时就注入此处的beanFactory对象，其它调用registerResolvableDependency的地方同理
		 */
		beanFactory.registerResolvableDependency(BeanFactory.class, beanFactory);
		/**
		 * 注入ResourceLoader的地方就注入Spring容器
		 */
		beanFactory.registerResolvableDependency(ResourceLoader.class, this);
		/**
		 * 注入ApplicationEventPublisher的地方就注入Spring容器
		 */
		beanFactory.registerResolvableDependency(ApplicationEventPublisher.class, this);
		/**
		 * 注入ApplicationContext的地方就注入Spring容器
		 */
		 ...
		 * 当前层不包含名为environment的Bean则给当前层注入单例Environment Bean，名为environment，其它三个if同理
		 */
	 ...
	 * 在内部应用上下文的bean工厂标准初始化之后进行修改的入口方法。此方法调用时所有的bean定义对象都已加载，但还没有实例化。
	 * 这允许某些ApplicationContext实现中注册一些特殊的BeanPostProcessors。
	 *
	...
	/**
	 * Initialize the ApplicationEventMulticaster.初始化应用事件多播广播器
	 * Uses SimpleApplicationEventMulticaster if none defined in the context.
	 * 如果上下文中没有设置事件广播器，使用SimpleApplicationEventMulticaster作为默认的广播器
	 *
	 * @see org.springframework.context.event.SimpleApplicationEventMulticaster
	 */
	...
	protected void finishBeanFactoryInitialization(ConfigurableListableBeanFactory beanFactory) {
		...
		// todo:very import 实例化所有剩余的非懒加载的单例Bean
	...
	protected void finishRefresh() {
		 ...
		 * 清理容器级别的资源缓存（比如ASM扫描后的元数据）
		 * Clear all resource caches in this resource loader.
		 */
		clearResourceCaches();

		/**
		 * Initialize lifecycle processor for this context.初始化容器的生命周期处理器
		 * Initialize the LifecycleProcessor. Uses DefaultLifecycleProcessor if none defined in the context.
		 * 初始化LifecycleProcessor，如果容器中没有定义LifecycleProcessor则使用DefaultLifecycleProcessor
		 */
